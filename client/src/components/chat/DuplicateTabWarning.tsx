...
/**
 * Renders a full-screen warning blocking the UI when a duplicate browser tab is detected.
 * This ensures WebRTC port binding and IndexedDB connections are not corrupted by race conditions.
 * Utilizes a Dark Glassmorphism aesthetic for premium UI consistency.
 *
 * @returns {React.JSX.Element} The duplicate tab warning UI.
 */
export default function DuplicateTabWarning(): React.JSX.Element {
  return (
    <div className="flex h-screen w-full flex-col items-center justify-center bg-zinc-950 p-4 font-sans selection:bg-indigo-500/30">
      ...
      <div className="relative z-10 w-full max-w-md bg-zinc-900/40 backdrop-blur-2xl border border-white/10 rounded-3xl shadow-2xl p-8 flex flex-col items-center text-center animate-in fade-in zoom-in-95 duration-300">
        ...
        <p className="text-sm text-zinc-400 leading-relaxed mb-6">
          A pure{" "}
          <span className="font-semibold text-zinc-300">Peer-to-Peer</span>{" "}
          communication system does not allow multi-tab usage to prevent WebRTC
          port conflicts and protect the integrity of your local cryptographic
          database.
        </p>
        ...
        <div className="bg-black/40 border border-zinc-800/50 rounded-xl p-4 w-full backdrop-blur-sm">
          <p className="text-xs text-zinc-300">
            ...
            return to the previously active application window.
          </p>
        </div>
      </div>
    </div>
